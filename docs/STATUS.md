...
完善个人主页专业技能图谱，围绕全栈开发与 Agent 工程两条主线呈现工程能力。

## 已完成

- 已移除首屏星轨周围的两个浅色圆环和一个浅色椭圆环，保留星轨、星点与银河粒子效果。
- 个人定位已统一收敛为“全栈开发 × Agent 工程”，中心说明聚焦前端应用、后端服务、智能体系统、业务与研发提效及端到端工程交付能力。
...
- Agent 工程技术难点标签已在保留“消息去重与状态同步”关键信息的前提下精简，并在桌面端保持同一行展示。
- 实习经历第 03 项已重构为平台稳定性专项，突出两大平台的问题分级与修复排期，改动涉及六个代码仓库，并覆盖前端、后端和数据库的 40+ 项问题修复上线。
- 业务数据已融入成果描述并强化展示，Smart Video 口径更新为上线 `20` 天累计获得 `1.2W+ PV`，并补充微前端、EDNX、DynoKit、Atlas、Jotai 技术标签。
...
- 悬浮详情已精简为一句高信息量的工程说明与技术要点，中心节点明确展示“全栈开发 × Agent 工程”。
